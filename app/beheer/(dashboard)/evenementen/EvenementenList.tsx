...
export function EvenementenList({ events }: { events: EventRow[] }) {
    ...
    return (
      <Card>
        <p className="text-ink-500">
          Nog geen evenementen. Voeg ze toe via de Supabase Table Editor — een
          schrijfinterface volgt in een latere fase.
        </p>
      </Card>
    );
  }
    ...
    <div className="space-y-3">
      ...
      {items.map((item) => (
        <Card key={item.id} className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h3 className="mb-1 font-serif text-base font-semibold text-forest-900">
              {item.titel}
            </h3>
            <p className="text-sm text-ink-500">
              {new Date(item.datum).toLocaleDateString("nl-NL")}
              {item.locatie ? ` · ${item.locatie}` : ""}
            </p>
          </div>
